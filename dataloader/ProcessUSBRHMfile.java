...
/*  PROGRAM  ProcessUSBRHMfile
	Initial coding BY M. Bogner Sutron Corporation
	initial code:  01-MARCH-2012

	This program is the java driver to process the file supplied by USBR Hydromet TAB
...
public class ProcessUSBRHMfile {
...

    {
     ...
     if (args.length == 1 && args[0].equalsIgnoreCase ("-V"))
     {
      ...
      if (args.length < 5) log_file = args[0] + ".log";
      else log_file = args[4];
 
      // create the logging file
      Logger log = Logger.createInstance(log_file);

      // now instantiate the USACE file reader and execute the process method for the USBR Hydromet file
