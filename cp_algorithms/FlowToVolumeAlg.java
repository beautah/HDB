...
MIN_VALUES_REQUIRED: Number: the required number of records that must exist for calculation to succeed
Value 0 means ALL interval observations must exist
...
OBSERVATIONS_CALCULATION : true/false : use exact number of interval observations to calculate flow
is only programmed to work for hourly and daily inputs! 

FLOW_FACTOR: Number: use as multiplier for flow to volume factor (2.0,1.98) : Default: 86400/43560 

NO_ROUNDING:  if no rounding in the calculation is desired; default is FALSE
 */
//AW:JAVADOC_END
public class FlowToVolumeAlg
...
// version 1.0.08 enhanced for LC request to choose if rounding desired by M. Bogner February 2013 
        String alg_ver = "1.0.08";
	String query;
...
//		first see if there are bad negative min settings for other than a monthly aggregate...
		if ( !output_interval.equalsIgnoreCase("month")) 
		{
		   if (mvr_count < 0 || mvd_count < 0)
		   {
		     ...
		     warning("FlowToVolumeAlg-"+alg_ver+" Warning: Illegal negative setting of minimum values criteria for non-Month aggregates");
		     warning("FlowToVolumeAlg-"+alg_ver+" Warning: Minimum values criteria for non-Month aggregates set to 1");
		   ...
		   if ((input_interval.equalsIgnoreCase("instant") || output_interval.equalsIgnoreCase("hour")) && mvr_count == 0) 
		   {
		     ...
		     warning("FlowToVolumeAlg-"+alg_ver+" Warning: Illegal zero setting of minimum values criteria for instant/hour aggregates");
		     warning("FlowToVolumeAlg-"+alg_ver+" Warning: Minimum values criteria for instant/hour aggregates set to 1");
		     mvr_count = 1;
		   }
		}
...
//		check and set minimums for monthly aggregates
		if ( output_interval.equalsIgnoreCase("month")) 
		{
		   ...
		   if (mvr_count <= 0 && input_interval.equalsIgnoreCase("hour")) mvr_count = days*24 + mvr_count;
		   if (mvd_count <= 0 && input_interval.equalsIgnoreCase("day")) mvd_count = days + mvd_count;
		   if (mvd_count <= 0 && input_interval.equalsIgnoreCase("hour")) mvd_count = days*24 + mvd_count;
		}
//
//		check and set minimums for daily aggregates
		if ( output_interval.equalsIgnoreCase("day")) 
		{
		   ...
		   {
		     warning("FlowToVolumeAlg-"+alg_ver+" Warning: Illegal zero setting of minimum values criteria for " 
		     + input_interval + " to daily aggregates");
		     warning("FlowToVolumeAlg-"+alg_ver+" Warning: Minimum values criteria for daily aggregates set to 1");
		     if (mvd_count == 0) mvd_count = 1;
		     if (mvr_count == 0) mvr_count = 1;
		   }
...
//
		...
		DataObject dbobj = new DataObject();
                ...
                Integer sdi = new Integer((int) getSDI("input"));
		...
		String status = null;
                ...
                debug3(" Total Count: " + total_count + "   daily_count: " + daily_count + "   hourly Count: " + hourly_count);
		String day_multiplier = 
                   	" ( to_date('" +   (String) dbobj.get("SD_EDT")  + "','dd-MM-yyyy HH24:MI') - " +
		   	"to_date('" +  (String) dbobj.get("SD_SDT") + "','dd-MM-yyyy HH24:MI') )"; 

		if (observations_calculation && input_interval.equalsIgnoreCase("day")) day_multiplier = " " + daily_count;
		if (observations_calculation && input_interval.equalsIgnoreCase("hour")) day_multiplier = " " + hourly_count;
		...
		String query = "select round( hdb_utilities.get_sdi_unit_factor( " + getSDI("input") + 
			...
			"hdb_utilities.date_in_window('" + output_interval.toLowerCase() +
                ...
                // if rounding not requested modify the query to not include rounding function
...
//
		debug3("FlowToVolumeAlg-"+alg_ver + "  "  + _aggregatePeriodEnd + " SDI: " + getSDI("input") + "  MVR: " + mvr_count + " RecordCount: " + total_count);
		...
		if (!is_current_period && total_count < mvr_count)
                {
 		   do_setoutput = false;
		   debug1("FlowToVolumeAlg-"+alg_ver+":  Minimum required records not met for historic period: " + _aggregatePeriodEnd + " SDI: " + getSDI("input") + "  MVR: " + mvr_count + " RecordCount: " + total_count);
		}
		if (is_current_period && !partial_calculations && total_count < mvr_count)
                {
 		   do_setoutput = false;
		   debug1("FlowToVolumeAlg-"+alg_ver+":  Minimum required records not met for current period: " + _aggregatePeriodEnd + " SDI: " + getSDI("input") + "  MVR: " + mvr_count + " RecordCount: " + total_count);
		}
//
		//
